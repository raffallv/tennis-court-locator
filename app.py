...
APP_VERSION = "v20 (merge results across all mirrors)"
...
# Separate radii: GPS "near me" is meant to be broad (traveling, general
# area), while a ZIP code is a much smaller, specific area — using the same
# 25-mile radius for both pulled in results from unrelated neighboring ZIPs.
GPS_RADIUS_DEG = 0.4   # ~25 miles — broad "near me" search
ZIP_RADIUS_DEG = 0.15  # ~10 miles — the earlier 0/0 results were caused by a slow regex query timing out server-side, not by the radius; now that's fixed, no need for the wider 15mi net
...
def geocode_zip(zip_code_str):
    """Look up a US ZIP code's centroid. Tries a dedicated ZIP-lookup service
    first (fast, purpose-built, rarely rate-limited), falling back to
    Nominatim if that's unavailable. Returns (lat, lon) or None."""
    zip_code_str = zip_code_str.strip()

    # Nominatim first: it's OSM-sourced, same data our tennis-court search
    # uses, and matched the actual court locations correctly in testing.
    # Zippopotam.us is faster but its centroids can be stale for newer or
    # recently-split ZIP codes, which silently shifted searches away from
    # where the real courts are.
    try:
    ...
    try:
        resp = requests.get(
            f"https://api.zippopotam.us/us/{zip_code_str}",
            headers=APP_USER_AGENT,
            timeout=10
        )
...
def run_overpass_query(query):
    """Query every mirror (not just the first that responds) and merge their
    elements, deduplicated by (type, id). A single mirror can return a
    technically-valid but incomplete response for a given region — merging
    catches real data that a first-success-only approach would miss."""
    merged_elements = {}
    responded_mirrors = []

    for mirror_url in OVERPASS_MIRRORS:
        try:
            response = requests.post(mirror_url, data=query, headers=APP_USER_AGENT, timeout=50)
        ...
        responded_mirrors.append(mirror_url)
        for element in payload.get("elements", []):
            key = (element.get("type"), element.get("id"))
            if key not in merged_elements:
                merged_elements[key] = element

    if not responded_mirrors:
        return None, None

    return {"elements": list(merged_elements.values())}, responded_mirrors


@st.cache_data(ttl=3600)
def fetch_tennis_courts(south, west, north, east):
    # Exact tag match (not regex) — regex tag matching is much slower for
    # Overpass to evaluate and was likely causing silent server-side
    # timeouts on larger bounding boxes, which return a technically-valid
    # but empty response (no HTTP error) that looked identical to "no
    # courts here." Exact match covers the vast majority of real tagging.
    combined_query = f"""
    [out:json][timeout:45];
    (
    ...
    data, used_mirror = run_overpass_query(combined_query)
    ...
    # Overpass can return HTTP 200 with valid JSON but an embedded error —
    # e.g. a server-side query timeout — which looks identical to "zero
    # results" unless we specifically check for it.
    if data.get("remark"):
        st.warning(f"Overpass server note (may explain missing results): {data['remark']}")
    ...
    try:
        ...
        return result_df, total_found, used_mirror
    except Exception as e:
        st.error(f"Error processing data: {e}")
        return pd.DataFrame(), 0, used_mirror


point_mode = False
user_lat, user_lon = None, None
radius_deg = GPS_RADIUS_DEG
...
if search_mode == "My GPS Location" and gps and gps.get("latitude") and gps.get("longitude"):
    ...
    if coords is None:
        st.error(
            f"Couldn't look up ZIP code '{zip_code}' right now. This is usually a "
            "temporary lookup-service hiccup, not an invalid ZIP — try the "
            "'Clear cache & retry' button in the sidebar."
        )
        st.stop()
    ...
    zoom_level = 12
    radius_deg = ZIP_RADIUS_DEG
    st.caption(f"Geocoded ZIP {zip_code} to ({user_lat:.4f}, {user_lon:.4f}), searching a {radius_deg*69:.0f}-mile-ish box around it.")
...
if point_mode:
    south, north = user_lat - radius_deg, user_lat + radius_deg
    west, east = user_lon - radius_deg, user_lon + radius_deg
else:
    south, west, north, east = STATE_BOUNDS[selected_state]

with st.spinner(f"Querying nationwide database near {location_label}..."):
    df, total_found, used_mirror = fetch_tennis_courts(south, west, north, east)

servers_text = ", ".join(used_mirror) if used_mirror else "none responded"
st.caption(f"Found {total_found} total tennis court entries near {location_label}; {len(df)} have a verifiable name, park, or website. (Data servers queried: {servers_text})")
