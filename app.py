...
APP_VERSION = "v5 (zip search + hardened theme)"
...
# Clay-court themed background — multiple selectors to cover different
# Streamlit versions (stApp vs stAppViewContainer vs stMain).
st.markdown(
    """
    <style>
    html, body,
    ...
    /* Make the state selector pop with grass green */
    [data-testid="stSidebar"] [data-testid="stSelectbox"]:nth-of-type(1) div[data-baseweb="select"] > div {
        background-color: #3C9A40 !important;
        border: 1px solid #2C7A30 !important;
        color: #FFFFFF !important;
        font-weight: 600;
    }
    [data-testid="stSidebar"] [data-testid="stSelectbox"]:nth-of-type(1) svg {
...
def fetch_tennis_courts(south, west, north, east):
    ...
    for mirror_url in OVERPASS_MIRRORS:
        try:
            response = requests.post(
                mirror_url, data=overpass_query, headers=APP_USER_AGENT, timeout=25
            )
    ...
    try:
        ...
        for element in data.get("elements", []):
            ...
            # Skip unverifiable entries: keep only courts with a real name
            # (a named park, club, or facility) OR a website to call/check.
...
if search_mode == "My GPS Location" and gps and gps.get("latitude") and gps.get("longitude"):
    ...
    # ZIP Code or GPS mode selected but no valid input yet
