...
APP_VERSION = "v19 (mirror attribution diagnostics)"
    ...
    for mirror_url in OVERPASS_MIRRORS:
        try:
            ...
            if response.status_code != 200 or "json" not in content_type:
                continue
            return response.json(), mirror_url
        except (requests.exceptions.RequestException, ValueError):
            continue
    return None, None
...
st.caption(f"Found {total_found} total tennis court entries near {location_label}; {len(df)} have a verifiable name, park, or website. (Data server: {used_mirror or 'none responded'})")
