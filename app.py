...
APP_VERSION = "v14 (Google Places API)"
...
st.markdown("Find, filter, and map real, verified tennis facilities across the US — powered by Google Places.")
...
# Search radii for point-based searches (Google's Nearby Search max is 50,000m)
GPS_RADIUS_M = 40000   # ~25 miles — broad "near me" search
ZIP_RADIUS_M = 8000    # ~5 miles — approximates a single ZIP code's footprint
...
# Types that suggest the facility is indoors (a gym/fitness club housing courts)
...
if search_mode == "State":
    selected_state = st.sidebar.selectbox("Select State", sorted(STATE_NAMES), index=sorted(STATE_NAMES).index("Nevada"))
...
def geocode_zip(zip_code_str):
    ...
    first, falling back to Nominatim. Returns (lat, lon) or None."""
...
def search_nearby(lat, lon, radius_m):
    """Google Places Nearby Search (New) for tennis courts around a point."""
    ...
    places = resp.json().get("places", [])
    return _places_to_dataframe(places), None


@st.cache_data(ttl=3600)
def search_text(query):
    """Google Places Text Search (New) for tennis courts by free-text query."""
    if not GOOGLE_API_KEY:
        return pd.DataFrame(), "missing_key"
...
if search_mode == "My GPS Location" and gps and gps.get("latitude") and gps.get("longitude"):
    ...
    with st.spinner(f"Searching for tennis courts near {location_label}..."):
        df, error = search_nearby(user_lat, user_lon, GPS_RADIUS_M)
    ...
    if coords is None:
        st.error(
            f"Couldn't look up ZIP code '{zip_code}' right now. Try the "
    ...
    zoom_level = 12
    with st.spinner(f"Searching for tennis courts near {location_label}..."):
        df, error = search_nearby(user_lat, user_lon, ZIP_RADIUS_M)
...
if error == "missing_key":
    ...
    st.error(f"Couldn't reach Google Places right now. Try 'Clear cache & retry'.")
    st.caption(f"Details: {error}")
    st.stop()

st.caption(f"Found {len(df)} verified tennis facilities near {location_label} (Text/Nearby Search results are capped at 20 per query by Google).")
...
if not df.empty:
    ...
    if point_mode:
        ...
        view_lat, view_lon = 39.5, -98.35  # geographic center of the US
